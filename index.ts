import {check, reserve, cancel} from './api';

const server = Bun.serve({
    port: 3000,
    fetch(req) {
        const url = new URL(req.url);
        // check could be a GET because we are not modifying anything on the system, just querying information.
        ...
        if (url.pathname === "/check" && req.method === "POST") return check(req);
        if (url.pathname === "/reserve" && req.method === "POST") return reserve(req);
